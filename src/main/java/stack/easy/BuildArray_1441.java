 ...
 * 示例 1：
 * <p>
 * 输入：target = [1,3], n = 3 输出：["Push","Push","Pop","Push"] 解释： 读取 1 并自动推入数组 -> [1] 读取 2 并自动推入数组，然后删除它 -> [1] 读取 3
 * 并自动推入数组 -> [1,3] 示例 2：
 * <p>
 * 输入：target = [1,2,3], n = 3 输出：["Push","Push","Push"] 示例 3：
 * <p>
 * 输入：target = [1,2], n = 4 输出：["Push","Push"] 解释：只需要读取前 2 个数字就可以停止。 示例 4：
 * <p>
 ...
 * <p>
 * 来源：力扣（LeetCode） 链接：https://leetcode-cn.com/problems/build-an-array-with-stack-operations
...
public class BuildArray_1441 {
    ...
    private List<String> fun(int[] target, int n) {
