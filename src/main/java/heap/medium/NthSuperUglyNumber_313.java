...
public class NthSuperUglyNumber_313 {
    ...
    @Test
    public void test() {
        int result = nthSuperUglyNumber(12, new int[]{2, 7, 13, 19});
    }
    ...
    /**
     * 使用动态规划 todo 执行部分结果报错
     */
    private int fun(int k, int[] primes) {
        int[] result = new int[k];
        result[0] = 1;
        int[] indexNum = new int[primes.length];
        for (int i = 1; i < k; i++) {
            int index = 0;
            int tempResult = Integer.MAX_VALUE;
            for (int j = 0; j < primes.length; j++) {
                int curValue = result[indexNum[j]] * primes[j];
                if (curValue < tempResult && curValue > result[i - 1]) {
                    tempResult = curValue;
                    index = j;
                }
            }
     ...
     * 参考思路
     */
    private int fun2(int n, int[] primes) {
