...
public class RankTeams_1366 {
    public static void main(String[] args) {
        ...
        RankTeams_1366 ceshi = new RankTeams_1366();
        System.out.println(ceshi.rankTeams(votes));
    }
    ...
    private String fun1(String[] votes) {
        int teamNum = votes[0].length();
        char[] resultChar = new char[teamNum];
        Map<Character, int[]> voteMap = new HashMap<>();
        for (String str : votes) {
            char[] strArr = str.toCharArray();
            for (int i = 0; i < strArr.length; i++) {
                if (!voteMap.containsKey(strArr[i])) {
                    voteMap.put(strArr[i], new int[teamNum]);
                }
                voteMap.get(strArr[i])[i]++;
            }
        }
        List<Map.Entry<Character, int[]>> entryList = new ArrayList<>();
        entryList.addAll(voteMap.entrySet());
            ...
            public int compare(Map.Entry<Character, int[]> o1, Map.Entry<Character, int[]> o2) {
                ...
                for (int i = 0; i < len; i++) {
                    if (o1.getValue()[i] > o2.getValue()[i]) {
                        return 1;
                    } else if (o1.getValue()[i] < o2.getValue()[i]) {
                        return -1;
                    }
                }
        ...
        for (int i = 0; i < teamNum; i++) {
            resultChar[i] = entryList.get(i).getKey();
        }
